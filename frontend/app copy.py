...
with st.expander("Betting Hub", expanded=True):
    ...
    def show_candidates(title, checklist, sort_cols, display_cols, rows=7):
        st.subheader(title)
        ...
        available_display = safe_cols(df, display_cols)
        available_sort = safe_cols(df, sort_cols)

        if len(available_display) <= 1:
            st.warning(f"{title} data unavailable.")
            return

        if available_sort:
            df = df.sort_values(available_sort, ascending=[False] * len(available_sort))

        st.dataframe(
            df[available_display].head(rows),
            use_container_width=True
        )
    ...
    with tabs[0]:
        show_candidates(
            ...
            ["xba", "hard_hit_pct", "avg_ev"],
            ["player_name", "xba", "hard_hit_pct", "avg_ev", "barrel_pct"],
            rows=7
        )
    ...
    with tabs[1]:
        show_candidates(
            ...
            ["player_name", "hard_hit_pct", "barrel_pct", "avg_ev", "max_ev"],
            rows=7
        )
    ...
    with tabs[2]: 
        show_candidates(
            ...
            ["barrel_pct", "avg_ev", "max_ev"],
            ["player_name", "pa", "avg_ev", "max_ev", "barrel_pct", "hard_hit_pct", "xslg", "iso"],
            rows=7
        )

    with tabs[3]:
        show_candidates(
            "Best 1.5 Hit Candidates",
            [
                "Contact Skill",
                "Recent Multi-Hit Form",
                "Low Strikeout %",
                "Lineup Spot",
                "Pitcher Contact Allowed",
                "Handedness Edge"
            ],
            ["xba", "avg_ev", "hard_hit_pct"],
            ["player_name", "xba", "avg_ev", "hard_hit_pct", "pa", "barrel_pct"],
            rows=7
        )
    ...
    with tabs[4]:
        show_candidates(
            ...
            ["xba", "avg_ev", "pa"],
            ["player_name", "pa", "xba", "avg_ev", "barrel_pct"],
            rows=7
        )
    ...
    with tabs[5]:
        show_candidates(
            ...
            [
                ...
                "Men-On Opportunity",
                "Park HR Factor",
                "Pitcher Run Allowed Profile",
                "Hard Hit %"
            ],
            ["barrel_pct", "hard_hit_pct", "avg_ev"],
            ["player_name", "pa", "avg_ev", "max_ev", "barrel_pct", "hard_hit_pct"],
            rows=7
        )
    ...
    with tabs[6]:
        ...
        st.info("""
Build small-card lotto ideas from stats only.

Use 3–5 legs max.

Good lotto structure:
• 1 safe singles candidate
• 1 total bases upside candidate
• 1 RBI candidate
• 1 runs candidate
• Optional 1.5 hits only if profile is strong

Avoid forcing plays. Only combine hitters with multiple stat edges.
""")

        lotto_cols = safe_cols(hitters, ["player_name", "xba", "avg_ev", "max_ev", "barrel_pct", "hard_hit_pct"])
        if lotto_cols:
            lotto = hitters[lotto_cols].head(7)
            st.dataframe(lotto, use_container_width=True)
        else:
            st.warning("Lotto builder data unavailable.")
...
7. Separate singles, doubles, total bases, hits, runs, RBIs.
...
""")
        ...
        challenge_cols = safe_cols(hitters, ["player_name", "pa", "xba", "avg_ev", "max_ev", "barrel_pct", "hard_hit_pct"])
        if challenge_cols:
            challenge = hitters[challenge_cols].head(7)
            st.dataframe(challenge, use_container_width=True)
        else:
            st.warning("Challenge data unavailable.")